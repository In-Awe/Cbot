        ...
        <tr className="border-b border-gray-700 hover:bg-gray-800/40">
            ...
            <td className="px-4 py-3">
                 <span className={`px-2 py-1 text-xs rounded-full font-medium ${
                    trade.status === 'active' ? 'bg-cyan-500/20 text-cyan-300' : 'bg-yellow-500/20 text-yellow-300'
                 }`}>
                    {trade.status}
                </span>
            </td>
            ...
            {isEditing ? (
                 <>
                    ...
                    <td className="px-4 py-3"><TableInput type="number" name="takeProfit" value={editState.takeProfit} onChange={handleInputChange} /></td>
                    <td className="px-4 py-3"><TableInput type="number" name="stopLoss" value={editState.stopLoss} onChange={handleInputChange} /></td>
                 </>
            ) : (
                <>
                    <td className="px-4 py-3 font-mono">${trade.entryPrice.toFixed(4)}</td>
                    <td className="px-4 py-3 font-mono text-green-400">${trade.takeProfit.toFixed(4)}</td>
                    <td className="px-4 py-3 font-mono text-red-400">${trade.stopLoss.toFixed(4)}</td>
                </>
            )}

            <td className="px-4 py-3 text-gray-400">{trade.openedAt.toLocaleTimeString()}</td>
                ...
                <div className="flex items-center justify-end gap-2">
                    ...
                    {trade.status === 'active' && (
                        isEditing ? (
                            ...
                            <Button onClick={() => setIsEditing(true)} variant="secondary" className="py-1 px-2 text-xs">Edit</Button>
                        )
                    )}
